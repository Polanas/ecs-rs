...
impl BlobVec {
    ...
    pub unsafe fn initialize_unchecked(&mut self, index: usize, value: OwningPtr<'_>) {
        ...
        let ptr = self.get_checked_mut(index);
    ...
    pub unsafe fn replace_unchecked(&mut self, index: usize, value: OwningPtr<'_>) {
        ...
        let destination = NonNull::from(self.get_checked_mut(index));
    ...
    pub unsafe fn swap_remove_and_forget_unchecked(&mut self, index: usize) -> OwningPtr<'_> {
        ...
        if index != new_len {
            std::ptr::swap_nonoverlapping::<u8>(
                self.get_checked_mut(index).as_ptr(),
                self.get_checked_mut(new_len).as_ptr(),
                size,
            );
        }
        self.len = new_len;
    ...
    pub unsafe fn swap_remove_unchecked(&mut self, index: usize, ptr: PtrMut<'_>) {
        debug_assert!(index < self.len());
        let last = self.get_checked_mut(self.len - 1).as_ptr();
        let target = self.get_checked_mut(index).as_ptr();
    ...
    /// # Safety
    #[inline]
    pub unsafe fn get_checked(&self, index: usize) -> Ptr<'_> {
    ...
    /// # Safety
    #[inline]
    pub unsafe fn get_checked_mut(&mut self, index: usize) -> PtrMut<'_> {
        assert!(index < self.len());
        let size = self.item_layout.size();
        // SAFETY:
        // - The caller ensures that `index` fits in this vector,
...
mod tests {
    ...
    unsafe fn get_mut<T>(blob_vec: &mut BlobVec, index: usize) -> &mut T {
        assert!(index < blob_vec.len());
        blob_vec.get_checked_mut(index).deref_mut::<T>()
    }
